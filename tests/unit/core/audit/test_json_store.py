"""Tests for JSON audit store (F-123)."""
...
def store(temp_store_path):
    """Create JSON store."""
    return JsonStore(temp_store_path)
...
class TestJsonStore:
    ...
    def test_initialization(self, temp_store_path):
        ...
        store = JsonStore(temp_store_path)

        assert store.records_dir.exists()
        assert store.records_dir.is_dir()
    ...
    def test_save_creates_file(self, store, sample_record):
        """Test saving creates JSON file."""
        store.save(sample_record)

        record_file = store.records_dir / f"{sample_record.audit_id}.json"
    ...
    def test_list_skips_invalid_json(self, store, sample_record):
        """Test listing skips invalid JSON files."""
        store.save(sample_record)

        # Create invalid JSON file
        invalid_file = store.records_dir / "invalid.json"
        invalid_file.write_text("{invalid json")

        # Should only return valid record
        records = store.list()
        assert len(records) == 1
        assert records[0].audit_id == sample_record.audit_id

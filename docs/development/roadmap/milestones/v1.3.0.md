...
|----|---------|----------|--------|
| [F-112](../features/completed/F-112-native-ollama-provider.md) | Native Ollama Provider | Provider | ✅ Complete |
| [F-113](../features/completed/F-113-pii-detection-anonymisation.md) | PII Detection & Anonymisation | Privacy | ✅ Complete |

## Success Criteria

- [x] `persona generate --provider ollama` works with local models
- [x] Auto-detection of available Ollama models
- [x] `persona generate --anonymise` removes PII from input data
- [x] Three anonymisation strategies: redact, replace, hash
- [x] `persona privacy scan` previews detected PII
- [x] Works offline (no cloud API calls required)
- [x] Test coverage >= 90% (2708 tests, 24 skipped for optional deps)
...
```
Input Data → presidio-analyzer → Entity Detection → Anonymisation Strategy → Safe Output
                    ↓                    ↓                    ↓
...
- [F-112: Native Ollama Provider](../features/completed/F-112-native-ollama-provider.md)
- [F-113: PII Detection & Anonymisation](../features/completed/F-113-pii-detection-anonymisation.md)
